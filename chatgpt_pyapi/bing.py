...
class Message:
    ...
    def from_api(cls, api_msg: dict):
        ...
        text = api_msg["item"]["messages"][1]["adaptiveCards"][0]["body"][0][
            "text"
        ].strip("\n")
        return cls(text, _role=_Roles.ASSISTANT)
...
class ChatGPT:
    ...
    def __init__(
        self, cookies_file_path: str, conversation_style=ConversationStyle.precise
    ):
        self.bot = _Chatbot(cookie_path=cookies_file_path)
        self.conversation_style = conversation_style
        self._message_history = []
        self._event_loop = asyncio.get_event_loop()

    def __del__(self):
        self._event_loop.run_until_complete(self.bot.close())
